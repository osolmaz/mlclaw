...
import { isMissingContainerError, isMissingVolumeError, mergeDockerLogStreams, withContext } from "../src/mlclaw/docker.js";

describe("Docker error matching", () => {
  it("matches missing resources regardless of Docker message casing", () => {
    expect(isMissingVolumeError(new Error("Error response from daemon: no such volume: mlclaw-test-live")))
      .toBe(true);
    ...
    expect(isMissingContainerError(new Error("Error response from daemon: no such object: mlclaw-test")))
      .toBe(true);
  });
