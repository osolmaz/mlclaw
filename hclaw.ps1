...
$MinNodeMajor = if ($env:HCLAW_MIN_NODE_MAJOR) { [int]$env:HCLAW_MIN_NODE_MAJOR } else { 22 }
$NodeVersion = if ($env:HCLAW_NODE_VERSION) { $env:HCLAW_NODE_VERSION } else { "v24.16.0" }
$PackageSpec = if ($env:HCLAW_NPM_SPEC) { $env:HCLAW_NPM_SPEC } else { "huggingclaw" }
$CacheRoot = if ($env:HCLAW_CACHE_DIR) {
  $env:HCLAW_CACHE_DIR
} elseif ($env:LOCALAPPDATA) {
  Join-Path $env:LOCALAPPDATA "huggingclaw"
} else {
  Join-Path $HOME ".cache\huggingclaw"
}
...
function Install-CachedNode {
  ...
  $tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("hclaw-node-" + [System.Guid]::NewGuid().ToString("N"))
  New-Item -ItemType Directory -Force -Path $tempDir | Out-Null
  try {
    $zipPath = Join-Path $tempDir "node.zip"
    $url = "https://nodejs.org/dist/$NodeVersion/$archiveName.zip"
    Write-Error "hclaw: installing Node.js $NodeVersion into $targetDir" -ErrorAction Continue
...
function Invoke-HuggingClaw($NodeBin, [string[]]$CliArgs) {
  ...
  if (-not (Test-Path $npmCli)) {
    ...
    & $npmCommand.Source exec --yes --package $PackageSpec -- huggingclaw @CliArgs
    exit $LASTEXITCODE
  }
  & $NodeBin $npmCli exec --yes --package $PackageSpec -- huggingclaw @CliArgs
  exit $LASTEXITCODE
}
...
Invoke-HuggingClaw (Join-Path $nodeDir "node.exe") $args
